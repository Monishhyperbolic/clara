...
export default function LongevityPage() {
  ...
  const impactColor = (i: string) => ({
    high: { bg: "var(--green)", text: "var(--cream)" },
    medium: { bg: "#FDF5E0", text: "#8A6B00" },
    low: { bg: "var(--cream-dark)", text: "var(--text-muted)" },
  }[i] ?? { bg: "var(--cream-dark)", text: "var(--text-muted)" });

  const Field = ({ label, k, type = "text", options }: { label: string; k: string; type?: string; options?: string[] }) => (
    <div>
      <label style={{ fontSize: "0.78rem", fontWeight: 500, color: "var(--text-muted)", display: "block", marginBottom: "0.3rem" }}>{label}</label>
      {options ? (
        <select value={metrics[k as keyof typeof metrics]} onChange={e => set(k, e.target.value)} style={{ width: "100%", border: "1.5px solid var(--border)", borderRadius: 8, padding: "0.55rem 0.75rem", fontFamily: "'DM Sans',sans-serif", fontSize: "0.85rem", color: "var(--text)", background: "var(--cream)", outline: "none" }}>
          {options.map(o => <option key={o}>{o}</option>)}
        </select>
      ) : (
        <input type={type} value={metrics[k as keyof typeof metrics]} onChange={e => set(k, e.target.value)} style={{ width: "100%", border: "1.5px solid var(--border)", borderRadius: 8, padding: "0.55rem 0.75rem", fontFamily: "'DM Sans',sans-serif", fontSize: "0.85rem", color: "var(--text)", background: "var(--cream)", outline: "none" }} />
      )}
    </div>
  );
  ...
  return (
    <div style={{ padding: "2.5rem 2.5rem 4rem" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.95rem", marginTop: "0.25rem" }}>Enter your biomarkers and lifestyle data to get your biological age estimate and personalised interventions.</p>
      </div>

      {!report && (
        <div style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", padding: "2rem" }}>
          <div style={{ fontWeight: 500, fontSize: "0.95rem", color: "var(--text)", marginBottom: "1.25rem" }}>Your Health Metrics</div>

          <div style={{ marginBottom: "1.25rem" }}>
            <div style={{ fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" as const, color: "var(--text-muted)", marginBottom: "0.75rem" }}>Demographics</div>
          ...
          <div style={{ marginBottom: "1.25rem" }}>
            <div style={{ fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" as const, color: "var(--text-muted)", marginBottom: "0.75rem" }}>Cardiovascular</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.75rem" }}>
          ...
          <div style={{ marginBottom: "1.25rem" }}>
            <div style={{ fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" as const, color: "var(--text-muted)", marginBottom: "0.75rem" }}>Metabolic & Hormonal</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.75rem" }}>
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            <div style={{ fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" as const, color: "var(--text-muted)", marginBottom: "0.75rem" }}>Lifestyle</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.75rem" }}>
          ...
          <button onClick={analyze} disabled={loading} style={{ width: "100%", background: "var(--green)", color: "var(--cream)", border: "none", borderRadius: 12, padding: "0.875rem", fontSize: "0.95rem", fontWeight: 500, cursor: "pointer" }}>
        ...
        <div>
          ...
          <div style={{ background: "var(--green)", borderRadius: 24, padding: "2.5rem", marginBottom: "1.5rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem", alignItems: "center" }}>
            <div>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: "0.5rem", marginBottom: "1rem" }}>
                <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: "5rem", lineHeight: 1, color: "var(--cream)", letterSpacing: "-0.04em" }}>{report.longevityScore}</span>
              ...
              <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 6, height: 8, marginBottom: "1.25rem" }}>
                <div style={{ background: "var(--cream)", width: `${report.longevityScore}%`, height: "100%", borderRadius: 6, transition: "width 1s ease" }} />
              </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 16, padding: "1.5rem" }}>
              ...
              <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: "3.5rem", color: "var(--cream)", letterSpacing: "-0.03em", marginBottom: "0.25rem" }}>{report.biologicalAge}</div>
              ...
              <button onClick={() => setReport(null)} style={{ marginTop: "1rem", background: "rgba(255,255,255,0.15)", color: "var(--cream)", border: "none", borderRadius: 100, padding: "0.5rem 1.25rem", fontSize: "0.8rem", cursor: "pointer" }}>Recalculate</button>
            </div>
          </div>
            ...
            {report.categories?.map((cat, i) => (
              <div key={i} style={{ background: "white", borderRadius: 16, border: "1px solid var(--border)", padding: "1.25rem" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}>
                  <div style={{ flex: 1, background: "var(--cream-dark)", borderRadius: 4, height: 6 }}>
                    <div style={{ width: `${cat.score}%`, height: "100%", background: "var(--green-light)", borderRadius: 4 }} />
                  </div>
                  <span style={{ fontWeight: 500, fontSize: "0.88rem", color: "var(--text)" }}>{cat.score}</span>
                </div>
                <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", lineHeight: 1.5 }}>{cat.summary}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "1.5rem" }}>
            <div style={{ background: "var(--green-muted)", borderRadius: 16, padding: "1.5rem" }}>
              ...
              {report.topWins?.map((w, i) => <div key={i} style={{ fontSize: "0.85rem", color: "var(--text-mid)", marginBottom: "0.6rem" }}>• {w}</div>)}
            </div>
            <div style={{ background: "#FDECD9", borderRadius: 16, padding: "1.5rem" }}>
              ...
              {report.topRisks?.map((r, i) => <div key={i} style={{ fontSize: "0.85rem", color: "var(--text-mid)", marginBottom: "0.6rem" }}>• {r}</div>)}
            </div>
          </div>

          {/* Interventions */}
          <div style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", padding: "1.75rem" }}>
            <div style={{ fontWeight: 500, fontSize: "0.95rem", color: "var(--text)", marginBottom: "0.25rem" }}>Personalised Interventions</div>
            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: "1.25rem" }}>Evidence-based actions ranked by impact on your longevity score</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "1rem" }}>
              {report.interventions?.map((iv, i) => (
                <div key={i} style={{ background: "var(--cream)", borderRadius: 12, padding: "1rem" }}>
                  ...
                  <div style={{ fontSize: "0.82rem", color: "var(--text-muted)", lineHeight: 1.5 }}>{iv.description}</div>
                </div>
              ))}
            </div>
          </div>
