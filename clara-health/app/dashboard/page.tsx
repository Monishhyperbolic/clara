...
const quickActions = [
  { label: "Chat with Clara", icon: "💬", href: "/dashboard/chat", desc: "Ask anything about your health" },
    ...
    <div style={{ padding: "2.5rem 2.5rem 4rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem", marginBottom: "2rem" }}>
        {quickActions.map(({ label, icon, href, desc }) => (
          <Link key={href} href={href} style={{ background: "white", borderRadius: 16, border: "1px solid var(--border)", padding: "1.25rem", textDecoration: "none", display: "block", transition: "transform 0.15s, box-shadow 0.15s" }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 8px 24px rgba(30,61,47,0.1)"; }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
        ...
        <div style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", overflow: "hidden" }}>
          <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontWeight: 500, fontSize: "0.95rem", color: "var(--text)" }}>Recent Lab Results</div>
          ...
          <div style={{ padding: "0.5rem 0" }}>
            {recentLabs.map(lab => (
              <div key={lab.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 1.5rem", borderBottom: "1px solid var(--border)" }}>
                <div>
                  <div style={{ fontSize: "0.88rem", fontWeight: 500, color: "var(--text)" }}>{lab.name}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                  ...
                  <span style={{
                    fontSize: "0.7rem", padding: "2px 8px", borderRadius: 100, fontWeight: 500,
                    background: lab.status === "high" ? "#FDECD9" : lab.status === "low" ? "#EDE7DC" : "var(--green-muted)",
                    color: lab.status === "high" ? "#C8632A" : lab.status === "low" ? "#8A6B00" : "var(--green)"
                  }}>
                    {lab.status === "high" ? "↑ High" : lab.status === "low" ? "↓ Low" : "✓ Normal"}
                  </span>
                </div>
              </div>
            ))}
          ...
          <div style={{ padding: "1rem 1.5rem" }}>
            <Link href="/dashboard/labs" style={{ display: "block", textAlign: "center" as const, background: "var(--green-muted)", color: "var(--green)", padding: "0.6rem", borderRadius: 10, fontSize: "0.85rem", fontWeight: 500, textDecoration: "none" }}>
              Analyze new labs
            </Link>
          </div>
        </div>
        ...
        <div style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", overflow: "hidden" }}>
          <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontWeight: 500, fontSize: "0.95rem", color: "var(--text)" }}>Active Medications</div>
          ...
          <div style={{ padding: "0.5rem 0" }}>
            {meds.map(med => (
              <div key={med.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.875rem 1.5rem", borderBottom: "1px solid var(--border)" }}>
                <div>
                  <div style={{ fontSize: "0.88rem", fontWeight: 500, color: "var(--text)" }}>{med.name}</div>
                  <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>{med.dose}</div>
                </div>
                <div style={{ textAlign: "right" as const }}>
                  <span style={{
                    fontSize: "0.72rem", padding: "3px 8px", borderRadius: 100, fontWeight: 500,
                    background: med.status === "refill" ? "#FDF5E0" : "var(--green-muted)",
                    color: med.status === "refill" ? "#8A6B00" : "var(--green)"
                  }}>
          ...
          <div style={{ padding: "1rem 1.5rem" }}>
            <Link href="/dashboard/prescriptions" style={{ display: "block", textAlign: "center" as const, background: "var(--green-muted)", color: "var(--green)", padding: "0.6rem", borderRadius: 10, fontSize: "0.85rem", fontWeight: 500, textDecoration: "none" }}>
              Request refill
            </Link>
          </div>
        </div>
        ...
        {/* Health Score */}
        <div style={{ background: "var(--green)", borderRadius: 20, padding: "1.5rem", color: "var(--cream)" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: "0.75rem", marginBottom: "1rem" }}>
            <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: "4rem", lineHeight: 1, letterSpacing: "-0.03em" }}>74</span>
          ...
          <div style={{ fontSize: "0.85rem", color: "rgba(255,255,255,0.7)", marginBottom: "1.25rem" }}>
            Estimated biological age: <strong style={{ color: "var(--cream)" }}>31 years</strong> (2 years younger than chronological)
          </div>
          <Link href="/dashboard/longevity" style={{ display: "inline-block", background: "rgba(255,255,255,0.15)", color: "var(--cream)", padding: "0.6rem 1.25rem", borderRadius: 100, fontSize: "0.82rem", textDecoration: "none" }}>
            View full report →
          </Link>
        </div>
        ...
        <div style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", padding: "1.5rem" }}>
          <div style={{ fontWeight: 500, fontSize: "0.95rem", color: "var(--text)", marginBottom: "0.25rem" }}>Clara's Recommendations</div>
          <div style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginBottom: "1.25rem" }}>Based on your recent results</div>
          {[
            { icon: "💊", text: "Discuss statin therapy with your clinician — LDL is above goal for your risk profile.", urgency: "soon" },
            { icon: "🥗", text: "Increase omega-3 intake through diet or supplementation to support lipid profile.", urgency: "routine" },
            { icon: "🔬", text: "Repeat lipid panel in 3 months after dietary changes.", urgency: "routine" },
            { icon: "🏃", text: "Add 30 min of moderate cardio 3x/week — strong evidence for LDL reduction.", urgency: "routine" },
          ].map((r, i) => (
            <div key={i} style={{ display: "flex", gap: "0.75rem", marginBottom: "0.875rem", padding: "0.75rem", background: "var(--cream)", borderRadius: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <span style={{ fontSize: "0.85rem", color: "var(--text)", lineHeight: 1.5 }}>{r.text}</span>
              </div>
              <span style={{ fontSize: "0.68rem", padding: "2px 8px", borderRadius: 100, fontWeight: 500, alignSelf: "flex-start", flexShrink: 0, background: r.urgency === "soon" ? "#FDF5E0" : "var(--green-muted)", color: r.urgency === "soon" ? "#8A6B00" : "var(--green)" }}>
                {r.urgency}
              </span>
            </div>
          ))}
