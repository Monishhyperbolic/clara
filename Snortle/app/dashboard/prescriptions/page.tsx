...
export default function PrescriptionsPage() {
  ...
  const statusColor = (s: string) => ({ pending: { bg: "#FDF5E0", text: "#8A6B00" }, approved: { bg: "var(--green-muted)", text: "var(--green)" }, rejected: { bg: "#FCEBEB", text: "#A32D2D" }, dispensed: { bg: "#E6F1FB", text: "#185FA5" } }[s] ?? { bg: "var(--parchment-3)", text: "var(--ink-muted)" });
  ...
  return (
    <div style={{ padding: "2.5rem 2.5rem 4rem" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--ink-muted)", fontSize: "0.95rem", marginTop: "0.25rem" }}>Manage active medications and request refills — reviewed by your clinician.</p>
      </div>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.75rem", background: "white", borderRadius: 12, padding: "0.3rem", border: "1px solid var(--border)", width: "fit-content" }}>
        {(["active","request","history"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: "0.5rem 1.25rem", borderRadius: 9, border: "none", cursor: "pointer", background: tab === t ? "var(--green)" : "transparent", color: tab === t ? "var(--parchment)" : "var(--ink-muted)", fontSize: "0.88rem", fontWeight: tab === t ? 500 : 400, transition: "all 0.15s" }}>
            ...
            <div key={i} style={{ background: "white", borderRadius: 16, border: "1px solid var(--border)", padding: "1.25rem 1.5rem", marginBottom: "1rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.78rem", color: "var(--ink-muted)", marginTop: 2 }}>{med.dose} · {med.prescriber}</div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                ...
                {med.status === "refill" && (
                  <button onClick={() => { setTab("request"); setForm(f => ({ ...f, medication: med.name })); }} style={{ background: "var(--green)", color: "var(--parchment)", border: "none", borderRadius: 100, padding: "0.4rem 1rem", fontSize: "0.8rem", cursor: "pointer" }}>Request refill</button>
                )}
              </div>
            </div>
          ))}
          <div style={{ background: "var(--parchment-3)", borderRadius: 12, padding: "1rem 1.5rem", fontSize: "0.82rem", color: "var(--ink-muted)" }}>🔒 All medications managed and reviewed by your assigned clinician.</div>
        </div>
      )}
        ...
        <div style={{ background: "white", borderRadius: 20, border: "1px solid var(--border)", padding: "2rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
            {[["Medication & Dose *","medication","e.g. Atorvastatin 20mg"],["Reason for Refill *","reason","e.g. Running low"],["Current Medications","currentMeds",""],["Known Allergies","allergies",""]].map(([label, key, ph]) => (
              <div key={key}>
                <label style={{ fontSize: "0.82rem", fontWeight: 500, color: "var(--ink-mid)", display: "block", marginBottom: "0.4rem" }}>{label}</label>
                <input value={form[key as keyof typeof form]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder={ph} style={{ width: "100%", border: "1.5px solid var(--border)", borderRadius: 10, padding: "0.65rem 0.875rem", fontFamily: "'Inter',sans-serif", fontSize: "0.88rem", background: "var(--parchment)", outline: "none" }} />
              </div>
            ))}
          </div>
          ...
          <button onClick={requestRefill} disabled={!form.medication || !form.reason || loading} style={{ width: "100%", background: form.medication && form.reason ? "var(--green)" : "var(--green-muted)", color: form.medication && form.reason ? "var(--parchment)" : "var(--ink-muted)", border: "none", borderRadius: 12, padding: "0.875rem", fontSize: "0.95rem", fontWeight: 500, cursor: "pointer" }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", background: "var(--green-muted)", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: "1.5rem" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "0.78rem", color: "var(--ink-muted)" }}>Will be saved and sent to your clinician for approval</div>
            </div>
          ...
          <div style={{ background: "white", borderRadius: 16, border: "1px solid var(--border)", padding: "1.5rem", marginBottom: "1rem" }}>
            <div style={{ fontSize: "0.8rem", fontWeight: 500, color: "var(--ink-muted)", marginBottom: "0.5rem", textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>Draft Clinical Note</div>
            <div style={{ background: "var(--parchment)", borderRadius: 10, padding: "1rem", fontSize: "0.88rem", lineHeight: 1.7, whiteSpace: "pre-wrap" as const }}>{draft.draftNote}</div>
          </div>
          {draft.interactions?.length > 0 && (
            <div style={{ background: "#FDF5E0", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: "1rem" }}>
              ...
              {draft.interactions.map((d, i) => <div key={i} style={{ fontSize: "0.85rem", color: "var(--ink-mid)" }}>• {d}</div>)}
            </div>
          )}
          {error && <div style={{ padding: "0.75rem", background: "#FCEBEB", borderRadius: 10, fontSize: "0.85rem", color: "#A32D2D", marginBottom: "1rem" }}>{error}</div>}
          <div style={{ display: "flex", gap: "1rem" }}>
            <button onClick={() => setDraft(null)} style={{ flex: 1, background: "var(--parchment-3)", color: "var(--ink-mid)", border: "none", borderRadius: 12, padding: "0.875rem", fontSize: "0.92rem", cursor: "pointer" }}>Edit request</button>
            <button onClick={submitRefill} disabled={saving} style={{ flex: 2, background: "var(--green)", color: "var(--parchment)", border: "none", borderRadius: 12, padding: "0.875rem", fontSize: "0.92rem", fontWeight: 500, cursor: "pointer" }}>
      ...
      {submitted && (
        <div style={{ background: "var(--green)", borderRadius: 20, padding: "3rem", textAlign: "center" as const }}>
          ...
          <h2 style={{ fontFamily: "'DM Serif Display',serif", fontSize: "1.8rem", color: "var(--parchment)", marginBottom: "0.75rem" }}>Request submitted</h2>
          ...
          <div style={{ display: "flex", gap: "1rem", justifyContent: "center" }}>
            <button onClick={() => { setSubmitted(false); setDraft(null); setForm(f => ({ ...f, medication: "", reason: "" })); setTab("active"); }} style={{ background: "var(--parchment)", color: "var(--green)", border: "none", borderRadius: 100, padding: "0.75rem 1.5rem", fontSize: "0.9rem", fontWeight: 500, cursor: "pointer" }}>Back to medications</button>
            <button onClick={() => { setSubmitted(false); setDraft(null); setForm(f => ({ ...f, medication: "", reason: "" })); setTab("history"); }} style={{ background: "rgba(255,255,255,0.15)", color: "var(--parchment)", border: "none", borderRadius: 100, padding: "0.75rem 1.5rem", fontSize: "0.9rem", cursor: "pointer" }}>View my requests</button>
          </div>
        </div>
      )}
      ...
      {tab === "history" && (
        <div>
          {histLoading && <div style={{ textAlign: "center" as const, padding: "3rem", color: "var(--ink-muted)" }}>Loading...</div>}
          {!histLoading && history.length === 0 && (
            <div style={{ background: "white", borderRadius: 16, border: "1px solid var(--border)", padding: "3rem", textAlign: "center" as const, color: "var(--ink-muted)" }}>
