...
const faqs = [
  { q: "What is Snortle?", a: "Snortle is an AI-powered primary care platform combining a smart AI assistant with board-certified clinicians to help you manage your health — labs, prescriptions, longevity, and more. Available 24/7." },
  { q: "What can Snortle actually do for me?", a: "Analyze your lab results in plain language, track biomarkers over time, draft prescription refill requests for clinician review, import your full medical history from 150,000+ providers, and give personalized longevity and diet insights." },
  { q: "Why use Snortle instead of other AI apps?", a: "Most AI apps don't access your real medical records or connect to licensed clinicians. Snortle routes everything through board-certified providers so recommendations are grounded in your actual history." },
  ...
  return (
    <div style={{ background: "var(--cream)", minHeight: "100vh", overflowX: "hidden" }}>
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(247,243,238,0.88)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--border)", padding: "0 5vw", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: "1.5rem", color: "var(--green)" }}>Snortle</span>
        <div style={{ display: "flex", gap: "2rem", alignItems: "center" }}>
          ...
          <Link href="/auth/signup" style={{ background: "var(--green)", color: "var(--cream)", padding: "0.5rem 1.25rem", borderRadius: 100, fontSize: "0.875rem", fontWeight: 500, textDecoration: "none" }}>Get started</Link>
        </div>
      </nav>
      ...
      <section style={{ minHeight: "100vh", padding: "120px 5vw 80px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center", position: "relative" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
            <Link href="/auth/signup" style={{ background: "var(--green)", color: "var(--cream)", padding: "0.875rem 2rem", borderRadius: 100, fontSize: "0.95rem", fontWeight: 500, textDecoration: "none" }}>Get started free</Link>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.6rem", marginTop: "2rem" }}>
            {["🔬 Analyze Labs","💊 Refill Prescription","📈 Longevity","🥗 Diet","🩺 Symptoms"].map(p => (
              <Link key={p} href="/auth/signup" style={{ background: "white", border: "1px solid var(--border)", borderRadius: 100, padding: "0.45rem 1rem", fontSize: "0.82rem", color: "var(--text-mid)", textDecoration: "none" }}>{p}</Link>
            ))}
          </div>
        </div>
        <div style={{ position: "relative", display: "flex", justifyContent: "center", alignItems: "center" }}>
          <div style={{ background: "white", borderRadius: 24, border: "1px solid var(--border)", padding: "1.5rem", width: 340, boxShadow: "0 24px 60px rgba(30,61,47,0.12)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.25rem", paddingBottom: "1rem", borderBottom: "1px solid var(--border)" }}>
              ...
              <div><div style={{ fontWeight: 500, fontSize: "0.95rem" }}>Snortle</div><div style={{ fontSize: "0.75rem", color: "var(--green-light)" }}>● Online · Clinician on call</div></div>
            </div>
      ...
      <div id="features" style={{ padding: "0 4vw 80px" }}>
        <div style={{ background: "var(--green)", borderRadius: 32, padding: "80px 6vw", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontSize: "1rem", fontWeight: 300, color: "rgba(255,255,255,0.7)", lineHeight: 1.7, marginBottom: "2rem" }}>Verify once and Snortle pulls your complete picture from 150,000+ hospitals. No passwords, no portal logins.</p>
      ...
      <section style={{ padding: "80px 5vw", textAlign: "center" as const }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.5rem", maxWidth: 900, margin: "0 auto" }}>
          {[["24/7","Snortle is always available — no appointments needed"],["150k+","Connected hospitals for instant history import"],["100%","Clinician-reviewed before any prescription is sent"]].map(([n,l])=>(
      ...
      <div style={{ padding: "0 4vw 80px" }}>
        <div style={{ background: "var(--green)", borderRadius: 32, padding: "80px 6vw", textAlign: "center" as const }}>
          ...
          <p style={{ fontSize: "1rem", color: "rgba(255,255,255,0.65)", marginBottom: "2rem" }}>Join thousands who've made Snortle their first call — not their last resort.</p>
          <Link href="/auth/signup" style={{ background: "var(--cream)", color: "var(--green)", padding: "1rem 2.5rem", borderRadius: 100, fontSize: "1rem", fontWeight: 500, textDecoration: "none", display: "inline-block" }}>Chat with Snortle →</Link>
        </div>
      </div>
      ...
      <footer style={{ padding: "3rem 5vw 1.5rem", borderTop: "1px solid var(--border)", display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", gap: "3rem" }}>
        <div>
          <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: "1.5rem", color: "var(--green)", display: "block", marginBottom: "0.75rem" }}>Snortle</span>
      ...
      <div style={{ padding: "1.5rem 5vw", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", fontSize: "0.78rem", color: "var(--text-muted)", flexWrap: "wrap" as const, gap: "1rem" }}>
        ...
        <span>Snortle is an AI assistant, not a licensed clinician. All outputs are informational only.</span>
      </div>
    </div>
  );
}
